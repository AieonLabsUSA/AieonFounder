  ...
  useEffect(()=>{
    ...
    el.textContent=`
      ...
      *{box-sizing:border-box;margin:0;padding:0;}
      ...
      textarea:focus,input:focus{border-color:#1a56db !important;}
    `;
...
const CATEGORIES = [
  {
    id:"idea", icon:"💡", label:"Business Idea",
    desc:"Get your idea stress-tested before you commit.",
    advisors:["cfo","cmo","vc","devil"],
    questions:["Is this viable?","Is my market real?","Is this the right idea for me?","What am I missing?"],
    color:"#f59e0b",
    charLimit:2000,
    charHint:"~300 words",
  },
  {
    id:"pitch", icon:"🎯", label:"Pitch / Elevator Pitch",
    desc:"Is your pitch clear, compelling, and credible?",
    advisors:["cmo","vc","devil"],
    questions:["Is this clear?","Would an investor read further?","Does this make me sound credible?","What's the weakest line?"],
    color:"#1a56db",
    charLimit:1000,
    charHint:"~150 words — keep it tight",
  },
  {
    id:"copy", icon:"✍️", label:"Landing Page / Website Copy",
    desc:"Would a stranger understand what you do and want it?",
    advisors:["cmo","devil"],
    questions:["Is this clear to someone who's never heard of me?","Does this make them want to keep reading?","What's confusing?","What's the strongest line?"],
    color:"#0891b2",
    charLimit:3000,
    charHint:"~450 words",
  },
  {
    id:"email", icon:"📧", label:"Cold Email / Outreach",
    desc:"Would this get a response — or land in trash?",
    advisors:["cmo","devil"],
    questions:["Would you respond to this?","Is the ask clear?","Is this too long?","What would you change first?"],
    color:"#8b5cf6",
    charLimit:1500,
    charHint:"~220 words — emails should be short",
  },
  {
    id:"pricing", icon:"💰", label:"Pricing / Offer",
    desc:"Is this priced right, positioned right, and worth it?",
    advisors:["cfo","cmo","vc","devil"],
    questions:["Is this priced right for my market?","Does the value match the price?","Is this too cheap?","What would you charge?"],
    color:"#059669",
    charLimit:2000,
    charHint:"~300 words",
  },
  {
    id:"plan", icon:"🗺️", label:"Business Plan Section",
    desc:"Paste a full section — exec summary, market analysis, financials, or operations.",
    advisors:["cfo","cmo","legal","vc","devil"],
    questions:["Does this make sense?","What assumption is most dangerous here?","What's missing?","Is the logic sound?"],
    color:"#dc2626",
    charLimit:8000,
    charHint:"~1,200 words — enough for a full section",
  },
];
...
const MOCK_REVIEWS = {
  cfo: {
    emoji:"📊", name:"The CFO", color:"#3b6ef8",
    response:"Your pricing structure has a unit economics problem. If your service costs $200 to deliver and you're charging $250, you have a 20% margin — not enough to survive client acquisition costs, revisions, and operational overhead. I'd push this to $350 minimum and test whether demand holds. The market will tell you faster than your spreadsheet will.",
    verdict:"⚠️ Needs work", verdictColor:"#f59e0b",
  },
  cmo: {
    emoji:"📣", name:"The CMO", color:"#0891b2",
    response:"The first sentence is doing too much work. You're describing what you do before you've given me a reason to care. Lead with the problem you solve, not the service you offer. 'Tired of losing customers after the first purchase?' lands harder than 'We provide retention marketing services.' Your audience decides in 3 seconds. Give them a reason in the first line.",
    verdict:"✏️ Revise the lead", verdictColor:"#1a56db",
  },
  devil: {
    emoji:"😈", name:"Devil's Advocate", color:"#dc2626",
    response:"Three things you haven't addressed: Who else is doing this in your exact market, and why is your version better? Why would someone choose you over doing nothing? And what happens when a client asks for a portfolio? These aren't hypotheticals — they're the first three questions every serious buyer asks. If you don't have answers ready, you don't have a pitch. You have an intention.",
    verdict:"🔴 Address these first", verdictColor:"#dc2626",
  },
  vc: {
    emoji:"💼", name:"The Skeptical VC", color:"#d97706",
    response:"The market size claim is unsubstantiated. 'Millions of small businesses need this' is not a market. I need to see: the specific segment you're targeting, what they currently spend on this problem (or what they lose by not solving it), and how you get to $100K ARR. Without that, this is a service, not a scalable business. That's fine — but call it what it is.",
    verdict:"📊 Validate the market", verdictColor:"#d97706",
  },
  legal: {
    emoji:"⚖️", name:"Legal Counsel", color:"#7c3aed",
    response:"Before you send this proposal to a single client, you need a service agreement that covers: scope of work, payment terms, revision limits, intellectual property ownership, and a limitation of liability clause. Without these, your first difficult client engagement becomes a legal dispute. A $300 contract template from a business attorney pays for itself the first time a client disputes a deliverable.",
    verdict:"⚠️ Get this in writing", verdictColor:"#7c3aed",
  },
};

// ── Submission form ───────────────────────────────────────────────────
function SubmitForm({ onSubmit, tier, humanSlotsLeft=3 }){
  ...
  // charLimit now comes from cat.charLimit

  const cat = CATEGORIES.find(c=>c.id===category);
  const valid = category && content.trim().length > (cat?.id==="plan"?100:30);

  return(
    <div>
      {/* Category picker */}
      <div style={{marginBottom:20}}>
        <div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:10}}>What are you submitting?</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:8}}>
          {CATEGORIES.map(cat=>(
            <button key={cat.id} onClick={()=>{setCategory(cat.id);setFocusQ("");}} style={{
              padding:"12px 14px",borderRadius:12,border:"1.5px solid "+(category===cat.id?cat.color:C.border),
              background:category===cat.id?cat.color+"0e":C.white,
              cursor:"pointer",fontFamily:"inherit",textAlign:"left",transition:"all 0.15s",
            }}>
              <div style={{fontSize:18,marginBottom:4}}>{cat.icon}</div>
              <div style={{fontSize:13,fontWeight:700,color:category===cat.id?cat.color:C.text}}>{cat.label}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2,lineHeight:1.4}}>{cat.desc}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      {cat&&(
        <>
          ...
          <div style={{marginBottom:16}}>
            <div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:8}}>What do you most want feedback on? <span style={{color:C.muted,fontWeight:400}}>(optional)</span></div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {cat.questions.map(q=>(
                <button key={q} onClick={()=>setFocusQ(focusQ===q?"":q)} style={{
                  padding:"6px 12px",borderRadius:20,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",
                  border:"1.5px solid "+(focusQ===q?cat.color:C.border),
                  background:focusQ===q?cat.color+"12":C.white,
                  color:focusQ===q?cat.color:C.muted,transition:"all 0.15s",
                }}>{q}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{marginBottom:12}}>
            <div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:6}}>Any context your reviewers should know? <span style={{color:C.muted,fontWeight:400}}>(optional)</span></div>
            <input value={context} onChange={e=>setContext(e.target.value)} placeholder="e.g. My target customer is X, I'm in Newark, my budget is under $5K..." style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1.5px solid "+C.border,fontSize:13,color:C.text,background:C.offWhite}}/>
          </div>
          ...
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:6}}>
              <div>
                <div style={{fontSize:12,fontWeight:700,color:C.text}}>Paste what you want reviewed</div>
                <div style={{fontSize:11,color:C.muted,marginTop:2}}>Up to {cat.charLimit.toLocaleString()} characters · {cat.charHint}</div>
              </div>
              <div style={{fontSize:11,color:content.length>cat.charLimit*0.9?"#ef4444":content.length>cat.charLimit*0.7?"#f59e0b":C.muted,fontWeight:content.length>cat.charLimit*0.9?700:400}}>{content.length.toLocaleString()}/{cat.charLimit.toLocaleString()}</div>
            </div>
            <textarea value={content} onChange={e=>setContent(e.target.value.slice(0,cat.charLimit))} placeholder={"Paste your "+cat.label.toLowerCase()+" here..."} rows={cat.id==="plan"?16:cat.id==="copy"?10:8} style={{width:"100%",padding:"13px 14px",borderRadius:12,border:"1.5px solid "+C.border,fontSize:14,color:C.text,lineHeight:1.65,background:C.offWhite}}/>
          </div>
          ...
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{fontSize:12,fontWeight:700,color:C.text}}>Attach files <span style={{color:C.muted,fontWeight:400}}>(optional)</span></div>
              <div style={{fontSize:11,color:C.muted}}>Docs, screenshots, images — max 5 files</div>
            </div>
            <div
              onClick={()=>fileInputRef.current&&fileInputRef.current.click()}
              style={{border:"2px dashed "+C.border,borderRadius:12,padding:"16px",textAlign:"center",cursor:"pointer",background:C.offWhite,transition:"all 0.2s"}}
              onDragOver={e=>{e.preventDefault();}}
              onDrop={e=>{
                e.preventDefault();
                const dropped = Array.from(e.dataTransfer.files).slice(0,5-uploads.length);
                setUploads(u=>[...u,...dropped].slice(0,5));
              }}>
              <img src={REVIEW_LOGO} alt="AieonFounder" style={{width:32,height:32,objectFit:"contain",marginBottom:6,filter:"drop-shadow(0 0 6px rgba(26,86,219,0.4))",animation:"arcFloat 4s ease-in-out infinite",display:"inline-block"}}/>
              <div style={{fontSize:13,color:C.muted}}>Click to upload or drag and drop</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>PDF, DOC, PNG, JPG, JPEG</div>
              <input ref={fileInputRef} type="file" multiple accept=".pdf,.doc,.docx,.png,.jpg,.jpeg" style={{display:"none"}}
                onChange={e=>{
                  const selected = Array.from(e.target.files).slice(0,5-uploads.length);
                  setUploads(u=>[...u,...selected].slice(0,5));
                }}/>
            </div>
            {uploads.length>0&&(
              <div style={{marginTop:8,display:"flex",flexDirection:"column",gap:6}}>
                {uploads.map((file,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:C.white,borderRadius:8,border:"1px solid "+C.border}}>
                    <span style={{fontSize:14}}>{file.name.endsWith('.pdf')?'📄':file.type.startsWith('image/')? '🖼️':'📝'}</span>
                    <span style={{fontSize:12,color:C.text,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{file.name}</span>
                    <span style={{fontSize:11,color:C.muted}}>{(file.size/1024).toFixed(0)}KB</span>
                    <button onClick={e=>{e.stopPropagation();setUploads(u=>u.filter((_,j)=>j!==i));}} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:14,padding:"0 2px",fontFamily:"inherit"}}>✕</button>
                  </div>
                ))}
                <div style={{fontSize:11,color:C.muted}}>{uploads.length}/5 files attached</div>
              </div>
            )}
          </div>

          <div style={{background:C.offWhite,border:"1px solid "+C.border,borderRadius:12,padding:"12px 16px",marginBottom:16}}>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Your reviewers for this submission</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {cat.advisors.map(id=>{
                const r = MOCK_REVIEWS[id];
                return r?(
                  <div key={id} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 10px",borderRadius:20,background:"#fff",border:"1px solid "+C.border}}>
              ...
              {tier==="ignite"&&(
                <div
                  onClick={()=>humanSlotsLeft>0&&setRequestHuman(r=>!r)}
                  style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",borderRadius:20,
                    background:requestHuman?"#eff6ff":C.white,
                    border:"1.5px solid "+(requestHuman?"#3b6ef8":(humanSlotsLeft>0?C.border:"#f1f5f9")),
                    cursor:humanSlotsLeft>0?"pointer":"default",transition:"all 0.2s",userSelect:"none"}}>
                  <div style={{width:20,height:20,borderRadius:5,border:"2px solid "+(requestHuman?"#3b6ef8":"#cbd5e1"),background:requestHuman?"#3b6ef8":C.white,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,overflow:"hidden",transition:"all 0.2s"}}>
                    {requestHuman&&<img src={REVIEW_LOGO} alt="" style={{width:16,height:16,objectFit:"contain",filter:"brightness(10)"}}/>}
                  </div>
                  <div>
                    <span style={{fontSize:12,fontWeight:700,color:requestHuman?C.blue:(humanSlotsLeft>0?C.text:C.muted)}}>
                      Request human review
                    </span>
                    <span style={{fontSize:11,color:C.muted,marginLeft:6,fontWeight:400}}>
                      {humanSlotsLeft>0?"· "+humanSlotsLeft+" of 3 left this month":"· No slots remaining"}
                    </span>
                  </div>
                </div>
              )}
          ...
          <button onClick={()=>valid&&onSubmit({category,focusQ,content,context,cat,requestHuman,uploads})} disabled={!valid} style={{
            width:"100%",padding:"14px",borderRadius:12,
            background:valid?"linear-gradient(135deg,"+cat.color+","+cat.color+"bb)":"#e2e8f0",
            color:valid?"#fff":"#94a3b8",border:"none",fontSize:15,fontWeight:700,
            cursor:valid?"pointer":"default",fontFamily:"inherit",
            boxShadow:valid?"0 6px 24px "+cat.color+"44":"none",transition:"all 0.2s",
          }}>
            Submit for Review →
          </button>
          {!valid&&content.length>0&&content.length<30&&(
            <div style={{textAlign:"center",marginTop:8,fontSize:12,color:C.muted}}>Add a bit more content for a useful review (30+ characters)</div>
          )}
        </>
      )}
    </div>
...
// ── Review response ───────────────────────────────────────────────────
function ReviewResponse({ submission, tier, humanSlotsLeft=3 }){
  const [expanded, setExpanded] = useState(null);
  const cat = submission.cat;
  const advisorIds = cat.advisors;

  return(
    <div style={{animation:"fadeUp 0.5s ease"}}>
      <div style={{background:"#f0fdf4",border:"1.5px solid #bbf7d0",borderRadius:14,padding:"16px 18px",marginBottom:20,display:"flex",alignItems:"center",gap:12}}>
        <img src={REVIEW_LOGO} alt="AieonFounder" style={{width:36,height:36,objectFit:"contain",filter:"drop-shadow(0 0 8px rgba(5,150,105,0.5))",flexShrink:0}}/>
        <div>
          <div style={{fontSize:14,fontWeight:700,color:C.text}}>Review complete. +25 XP earned.</div>
          <div style={{fontSize:12,color:C.muted,marginTop:2}}>
            {advisorIds.length} AI advisors reviewed your {cat.label.toLowerCase()}.
            {tier==="ignite"&&" Your human review will arrive within 48 hours."}
          </div>
        </div>
      </div>

      {/* Submission context */}
      <div style={{background:C.offWhite,border:"1px solid "+C.border,borderRadius:12,padding:"12px 16px",marginBottom:20}}>
        <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>What you submitted</div>
        <div style={{fontSize:13,color:C.text,fontWeight:600,marginBottom:4}}>{cat.icon} {cat.label}{submission.focusQ&&<span style={{fontWeight:400,color:C.muted}}> · "{submission.focusQ}"</span>}</div>
        <div style={{fontSize:12,color:C.muted,lineHeight:1.6,maxHeight:60,overflow:"hidden",position:"relative"}}>
          {submission.content.slice(0,120)}...
        </div>
        {submission.uploads&&submission.uploads.length>0&&(
          <div style={{marginTop:8,display:"flex",gap:6,flexWrap:"wrap"}}>
            {submission.uploads.map((file,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 8px",background:C.offWhite,borderRadius:6,border:"1px solid "+C.border}}>
                <span style={{fontSize:11}}>{file.name.endsWith('.pdf')?'📄':file.type&&file.type.startsWith('image/')?'🖼️':'📝'}</span>
                <span style={{fontSize:11,color:C.muted}}>{file.name}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:20}}>
        {advisorIds.map((id,i)=>{
          ...
          return(
            <div key={id} style={{
              background:C.white,border:"1.5px solid "+(isOpen?review.color+"44":C.border),
              borderRadius:16,overflow:"hidden",transition:"border 0.2s",
              animation:"fadeUp 0.4s ease "+(i*0.1)+"s both",
            }}>
              <button onClick={()=>setExpanded(isOpen?null:id)} style={{width:"100%",padding:"14px 16px",background:"none",border:"none",display:"flex",alignItems:"center",gap:12,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
                <div style={{width:36,height:36,borderRadius:10,background:review.color+"15",border:"1px solid "+review.color+"33",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{review.emoji}</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:13,fontWeight:700,color:C.text}}>{review.name}</div>
                  <div style={{fontSize:12,fontWeight:700,color:review.verdictColor,marginTop:1}}>{review.verdict}</div>
                </div>
                <div style={{fontSize:11,color:C.muted,transform:isOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</div>
              </button>
              {isOpen&&(
                <div style={{padding:"0 16px 16px",borderTop:"1px solid "+C.border,paddingTop:14,animation:"fadeUp 0.3s ease"}}>
                  <p style={{fontSize:14,color:C.text,lineHeight:1.75}}>{review.response}</p>
                </div>
              )}
            </div>
          );
      ...
      {tier==="ignite"&&submission.requestHuman&&humanSlotsLeft>0&&(
        <div style={{background:"linear-gradient(135deg,#eff6ff,#f0f9ff)",border:"1.5px solid #bfdbfe",borderRadius:14,padding:"16px 18px",marginBottom:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:C.blue,animation:"blink 1.2s ease-in-out infinite"}}/>
            <div style={{fontSize:13,fontWeight:700,color:C.text}}>Human review queued · {humanSlotsLeft-1} slot{humanSlotsLeft-1===1?"":"s"} remaining this month</div>
          </div>
          <div style={{fontSize:12,color:C.muted,lineHeight:1.6}}>A member of the AieonLabs team will review your submission and add a personal note within 48 hours.</div>
        </div>
      )}
      {tier==="ignite"&&!submission.requestHuman&&humanSlotsLeft>0&&(
        <div style={{background:"#f8faff",border:"1px solid #e2e8f0",borderRadius:12,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:14}}>👤</span>
          <div style={{flex:1}}>
            <div style={{fontSize:12,color:C.muted}}>You didn't request a human review for this submission.</div>
            <div style={{fontSize:11,color:C.muted,marginTop:2}}>{humanSlotsLeft} slot{humanSlotsLeft===1?"":"s"} remaining this month — use them when it counts.</div>
          </div>
        </div>
      )}
      {tier==="ignite"&&humanSlotsLeft===0&&(
        <div style={{background:"#f8faff",border:"1.5px solid #e2e8f0",borderRadius:14,padding:"14px 18px",marginBottom:20}}>
          <div style={{fontSize:13,fontWeight:600,color:C.muted}}>You've used all 3 human review slots this month. Slots reset on your next billing date.</div>
        </div>
      )}

      {(tier==="free"||!tier)&&(
        <div style={{background:"linear-gradient(135deg,#080f2b,#0d1f4e)",borderRadius:14,padding:"16px 18px",textAlign:"center"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:4}}>Want a human to review this too?</div>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.5)",marginBottom:12}}>Ignite members get a personal note from the AieonLabs team within 48 hours.</div>
          <button style={{padding:"9px 24px",borderRadius:10,background:"linear-gradient(135deg,#1a56db,#00d4c8)",color:"#fff",border:"none",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Upgrade to Ignite →</button>
        </div>
      )}

      <button onClick={()=>window.location.reload()} style={{width:"100%",padding:"12px",borderRadius:12,background:C.white,border:"1.5px solid "+C.border,color:C.muted,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",marginTop:8}}>
        Submit Another →
      </button>
    </div>
  );
...
var REVIEW_LOGO = getLogo();

export default function ReviewDropbox(){
  const [submission, setSubmission] = useState(null);
  const [humanSlotsLeft, setHumanSlotsLeft] = useState(2); // mock: 2 of 3 remaining
  const [loading, setLoading] = useState(false);
  const tier = "ignite";

  const handleSubmit = (data) => {
    setLoading(true);
    setTimeout(()=>{ setLoading(false); setSubmission(data); }, 2200);
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:C.offWhite,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{background:C.navyDark,padding:"14px 24px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        <img src={REVIEW_LOGO} alt="AieonLabs" style={{width:24,height:24,objectFit:"contain",filter:"drop-shadow(0 0 6px rgba(0,212,200,0.6))"}}/>
        <div style={{fontSize:13,fontWeight:800,color:"#fff"}}>AieonFounder</div>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.3)"}}>·</div>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.4)"}}>Review Drop Box</div>
        <div style={{marginLeft:"auto",fontSize:11,color:"#3b6ef8",fontWeight:700}}>⚡ Ignite</div>
      </div>

      <div style={{maxWidth:680,margin:"0 auto",padding:"28px 20px"}}>
        <div style={{marginBottom:24}}>
          <div style={{fontSize:11,color:C.blue,letterSpacing:3,textTransform:"uppercase",fontFamily:"monospace",marginBottom:6}}>Review Drop Box</div>
          <h2 style={{fontSize:"clamp(20px,3vw,28px)",fontWeight:900,color:C.text,marginBottom:6}}>Drop it in. Get real feedback.</h2>
          <p style={{fontSize:14,color:C.muted,lineHeight:1.7}}>Paste your idea, pitch, copy, email, or plan. Your AI board reviews it instantly. Ignite members also get a personal human note within 48 hours.</p>
        </div>

        <div style={{background:C.white,borderRadius:20,border:"1.5px solid "+C.border,padding:"24px",boxShadow:"0 4px 24px rgba(0,0,0,0.05)"}}>
          {loading?(
            <div style={{textAlign:"center",padding:"48px 24px"}}>
              <div style={{marginBottom:16,animation:"arcFloat 4s ease-in-out infinite",display:"inline-block"}}>
              <img src={REVIEW_LOGO} alt="AieonFounder" style={{width:48,height:48,objectFit:"contain",filter:"drop-shadow(0 0 12px rgba(26,86,219,0.7))"}}/>
            </div>
            <div style={{fontSize:11,color:C.blue,letterSpacing:3,textTransform:"uppercase",fontFamily:"monospace",marginBottom:16}}>Reviewing...</div>
              <div style={{display:"flex",flexDirection:"column",gap:10,maxWidth:320,margin:"0 auto"}}>
                {["Analyzing your submission...","Routing to relevant advisors...","Generating structured feedback...","Finalizing board response..."].map((msg,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:C.offWhite,borderRadius:10,animation:"fadeUp 0.4s ease "+(i*0.4)+"s both"}}>
                    <div style={{width:7,height:7,borderRadius:"50%",background:C.blue,animation:"blink 1.2s ease-in-out "+(i*0.3)+"s infinite"}}/>
                    <span style={{fontSize:13,color:C.muted}}>{msg}</span>
                  </div>
                ))}
              </div>
            </div>
          ):submission?(
            <ReviewResponse submission={submission} tier={tier} humanSlotsLeft={humanSlotsLeft}/>
          ):(
            <SubmitForm onSubmit={handleSubmit} tier={tier} humanSlotsLeft={humanSlotsLeft}/>
          )}
        </div>

        {!submission&&!loading&&(
          <div style={{marginTop:16,display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
            {[{icon:"⚡",text:"AI review in ~30 seconds"},{icon:"👤",text:"3 human reviews/month (Ignite)"},{icon:"🔒",text:"Private — never shared"}].map((item,i)=>(
        ...
        <div style={{marginTop:20,padding:"14px 16px",background:"#f8faff",borderRadius:12,border:"1px solid #e2e8f0",fontSize:11,color:C.muted,lineHeight:1.7}}>
          <strong style={{color:C.text}}>⚠️ AI Disclaimer:</strong> All feedback generated by the AI board is for informational and educational purposes only. It does not constitute legal, financial, business, or professional advice. AI-generated responses may contain errors, omissions, or inaccuracies — including hallucinations (plausible-sounding but incorrect content). Always verify critical information with a qualified professional before making business decisions. AieonLabs and AieonFounder are not liable for outcomes resulting from AI-generated feedback.
        </div>
      </div>
    </div>
  );
